...
const TaggerData = () => {
  ...
  let currentPort;
  const [appPort2, setAppPort2]= useState("");

  const location = useLocation();
  const [appName, setAppName] = useState(''); // Define appName state variable
  const [ setAppPort] = useState(null)
      ...
      render: (text, record) => {
        try {
          ...
          console.log(item);
            ...
            console.log("appPort2:",appPort2);
            const otherAppUrl = `http://localhost:${appPort2}/${record.profile_id}/${record.task_mediatype}?username=${taggerId}&taggerName=${record.profile_fullname}`;
            return (
  ...
  const StatusSelect = ({ record }) => {
    ...
    axios.post(`${DOMAIN}/storePort`, { port: appPort })
      ...
      axios.get(`${DOMAIN}/getPort`, { port: appPort })
      .then((response) => {
        console.log("Response:", response.data.appPort);
        const Response = response.data.appPort;
        setAppPort2(Response);
